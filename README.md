...
> **"차량 등록 데이터로 측정하는 도시 환경오염의 지표"** > 서울시 25개 자치구의 자동차 연료별 환경오염을 시각화하여, 자동차 등록 증가가 환경에 주는 부담을 체감하게 하는 공감형 데이터 플랫폼입니다.
...
### 🎯 프로젝트 목표
1. **시각적 지표화** : 서울시 자치구별 자동차 연료 소비 데이터를 시각적으로 표현하여 대중교통 이용 유도.
2. **데이터 표준화** : 연료 종류별 소비량을 비교 가능하도록 수치화.
3. **지역 특성 파악** : 자치구별 자동차 에너지 소비 특성 분석.
